...
export function dateDiff(params: {
  ...
  try {
    ...
    return `${params.hasDay ? days : ""}${params.hasHour ? hours : ""}${
      params.hasMin ? minutes : ""
...
export function sendSms(phone: string, body: string = "你好") {
  ...
  window.location.href =
    platform === "ios"
      ? `sms:/open?addresses=${phone}&body=${body}`
      : `sms:${phone}?body=${body}`;
  // #endif
  // #ifdef APP-PLUS
