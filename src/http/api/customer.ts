...
// 客户管理在园客户列表
export function apiSearchInParkUserAndComany(data: IPageParams<TSearchInParkForm>) {
  return rjkRequest<IPageBody<ICostomer>>({
...
// 个人客户关联房产
export function apiGetRoomsByUserId(data: { id: number | string; parkIds: (number | string)[] }) {
...
// 个人客户关联房产
export function apiGetRoomsByCompanyId(data: { id: number; parkIds: (number | string)[] }) {
  return rjkRequest<TGetRoomsBy>({
...
// 修改个人手机号
export function apiModifyCustomerMobile(data: { id: number; mobile: string; checkCode: string; version: number }) {
  return rjkRequest({
...
// 修改用户信息
export function apiModifyUserInfo(data: { id: number; name: string; email: string; avatar: string; version: number }) {
  return rjkRequest({
...
// 园区和权限码搜索人员列表的接口 - cj
export function apiSearchParkAuthUser(data: { parkId: number | undefined; authCode: string }) {
  return rjkRequest({
...
export function apiGetCompanyLogList(
  data: IPageParams<{ id: number; startTime: string; endTime: string; types: number[] }>
) {
...
export function apiGetCompanyUserList(
  data: IPageParams<{ companyId: number; keywords: string; positionName: string }>
) {
  return rjkRequest<IPageBody<IUser>>({
...
// 获取企业的雇员列表
export function apiGetCompanyEmployeeList(data: IPageParams<{ companyId: number; keywords: string }>) {
  return rjkRequest<IPageBody<IUser>>({
...
// 个人设置登录名
export function apiSetLoginName(data: { id: number; loginName: string; version: number }) {
  return rjkRequest({
...
// 删除客户(在园/离园)
export function apiCompanyDeleteCompanyById(params: { customerType?: number; customerComId?: number }) {
  return rjkRequest<{
    data?: any[]
    unit: '0' | '1'
  }>({
