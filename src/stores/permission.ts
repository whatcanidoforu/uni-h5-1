...
export const usePermissionStore = defineStore(
  ...
  () => {
    ...
    const menuChildList = computed(() => {
      const _chilMenus = menuList.value.find((item) =>
    ...
    async function getUserAllPermission() {
      let list: string[] = [];
      const _userId = userId.value || uni.getStorageSync("userId");
      if (!_userId) {
        uni.setStorageSync("userPermission", JSON.stringify(list));
        return [];
      } else {
    ...
    async function getMenuList(routeObj: any = {}) {
      ...
      menusMap = {
        attractInvestment:
          menuData.value.find((item) => item.id == "attractInvestment")
            ?.children || [],
        propertyManager:
          menuData.value.find((item) => item.id == "propertyManager")
            ?.children || [],
      };
      menuList.value =
        menuData.value.find((item) => item.id == "attractInvestment")
