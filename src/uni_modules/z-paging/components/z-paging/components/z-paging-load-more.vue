...
<template>
    <view class="zp-l-container" :style="[zConfig.customStyle]" @click="doClick">
        <template v-if="!zConfig.hideContent">
            ...
            <!-- #ifndef APP-NVUE -->
            <image v-if="finalStatus===M.Loading&&!!zConfig.loadingIconCustomImage" :src="zConfig.loadingIconCustomImage" :style="[zConfig.iconCustomStyle]" :class="{'zp-l-line-loading-custom-image':true,'zp-l-line-loading-custom-image-animated':zConfig.loadingAnimated}" />
            <image v-if="finalStatus===M.Loading&&zConfig.loadingIconType==='flower'&&!zConfig.loadingIconCustomImage.length" class="zp-line-loading-image" :style="[zConfig.iconCustomStyle]" :src="isWhite?base64FlowerWhite:base64Flower" />
            <!-- #endif -->
            <!-- #ifdef APP-NVUE -->
            <view>
            ...
            <!-- #endif -->
            <text v-if="finalStatus===M.Loading&&zConfig.loadingIconType==='circle'&&!zConfig.loadingIconCustomImage.length" class="zp-l-line-loading-view" :style="[{borderColor:isWhite?'#aaaaaa':'#c8c8c8',borderTopColor:isWhite?'#ffffff':'#444444'},zConfig.iconCustomStyle]" />
