...
import { ref } from "vue";
import { apiSearchUserByKeyword } from "@/http/api/customer";
import { apiCompanyDepartmentGetMemberOfDepartments } from "@/http/api/contract";

const clueDirectorIdsCheckPopRef = ref();
const emit = defineEmits(["update:selectedSignBy", "confirm"]);
...
const apiSearchUserFun = () => {
  if (props.departments && props.departments.length) {
    const departmentIds = props.departments.map((item: any) => item.id);
    apiCompanyDepartmentGetMemberOfDepartments({
      departmentIdList: departmentIds,
      includeSub: false,
    }).then((res) => {
      directorsList.value = res.map((item: any) => {
        return {
...
const confirm = () => {
  emit("update:selectedSignBy", useDirectors.value);
...
const open = () => {
  apiSearchUserFun();
  useDirectors.value = [...props.selectedSignBy];
